...
/*   Updated: 2017/01/09 00:12:29 by cchaumar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static unsigned long	pieces[19] = {0xc000c000, 0xf000,
...
static int		sf_tets(int fd, t_tet *tet, t_point p, t_point deg)
{
	...
	{
		tet->blocs |= (unsigned long)(deg.x == BLOC) <<
			(63 - ((3 - p.y) << 4) - p.x);
		if ((p.x += (deg.x != LINE)) == 4)
			p = ft_point(0, p.y + 1);
		if (deg.x == LINE && p.y == 4)
		{
			...
			while (++p.y < 4 && (tet->ymax += (tet->ymax < 3 && tet->blocs &
				((unsigned long)0xF000 << ((tet->ymax + 1) << 4)))))
...
static int		sf_rec(unsigned short map[16], t_tet *tet, int size)
{
	...
	while (tet->ymax + (++tet->pos.y) < size)
	{
		...
		{
			if (!(*((unsigned long*)(map + tet->pos.y)) &
				(tet->blocs >> tet->pos.x)))
			{
				*((unsigned long*)(map + tet->pos.y)) ^=
					tet->blocs >> tet->pos.x;
